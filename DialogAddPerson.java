...
import package1.EyeColor;
import package1.Gender;

@SuppressWarnings("serial")
public class DialogAddPerson extends JDialog{
	 ...
	 *tree
	 ******************************************************************/
	public DialogAddPerson (JFrame parent, FamilyTree treeP, TreeLibrary libraryP) {
		...
		//Sets up correct pointers between the classes for communication
		tree = treeP;
	...
	private class ButtonListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if(btnBrowse == e.getSource()){
				JFileChooser c = new JFileChooser();
				int rVal = c.showOpenDialog(DialogAddPerson.this);
				if (rVal == JFileChooser.APPROVE_OPTION) {
					File file = c.getSelectedFile();
					imgFileField.setText(c.getSelectedFile().getPath());
					filename = file.getPath();
				}
				if (rVal == JFileChooser.CANCEL_OPTION) {
					imgFileField.setText("You pressed cancel");
				}
			}
						...
						if(ch == null){
							...
							try {
								...
								for(int i = 0; i < tokens.length; i++){
									...
									tree.addChild(child);
									if(tree.getRoot().getGender() == Gender.FEMALE){
										child.setMother(tree);
									}else{
										child.setFather(tree);
									}
								}
							} catch (Exception e1) {
